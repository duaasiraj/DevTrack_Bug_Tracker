...
import { X, LayoutDashboard, FolderKanban, Bell, ShieldAlert, LogOut, LayoutGrid, Bug } from 'lucide-react';
...
const Sidebar = ({ isOpen, toggleSidebar }) => {
  ...
  // ALL hooks must be at the top, before any other logic
  ...
  useEffect(() => {
    ...
    fetchUnread(); // run once immediately on mount

    const interval = setInterval(fetchUnread, 30000); // then every 30s

    return () => clearInterval(interval); // cleanup on unmount — IMPORTANT
        ...
        <div className="px-6 mb-8">
          ...
          {user && (
            <p className="text-[11px] text-gray-500 truncate mt-2" title={user.email}>
              {user.username}
              <span className="block text-[#78e5ef]/60 capitalize">
        ...
        <nav className="flex-1 space-y-1 px-4">
          {navItems.map((item) => (
            ...
            >
              ...
              {/* Unread badge — only shows on the Notifications link */}
              {item.name === 'Notifications' && unreadCount > 0 && (
                <span className="ml-auto text-[10px] bg-[#78e5ef] text-[#042124] font-bold rounded-full w-5 h-5 flex items-center justify-center shrink-0">
